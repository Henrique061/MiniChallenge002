...
class MenuMudancaFase: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        addChild(botaoJogar())
        addChild(criarLixeira())
    }

    // criando o botao de jogar
    func botaoJogar() -> SKNode  {
        let botaoJogar = ButtonPrefab(positionPoint: CGPoint(x: 250, y: -100), spriteWidth: 150, labelText: "Jogar", fontSize: 25) {
    ...
    func criarLixeira() -> SKNode {
        let lixeira = ButtonPrefab(color: .orange, colorSwitch: .orange, positionPoint: CGPoint(x: -200, y: -100), labelText: "Lixeira", rectangleSize: CGSize(width: 150, height: 200)) {
